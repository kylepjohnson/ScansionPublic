...
punc = '#$%^&*()_+={}[]|:;"\'\/,<>`~'
numbers = '1234567890'
abrev = ['Agr.', 'Ap.', 'A.', 'K.', 'D.', 'F.', 'C.', 'Cn.', 'L.', 'Mam.', 'M\'', 'M.', 'N.', 'Oct.', 'Opet.', 'Post.', 'Pro.', 'P.', 'Q.', 'Sert.', 'Ser.', 'Sex.', 'S.', 'St.', 'Ti.', 'T.', 'V.', 'Vol.', 'Vop.', 'Pl.'] #This is an exhaustive list of Latin praenomen abreviations.
vowels = ['a', 'e', 'i', 'o', 'u']
...
def tokenize(text):
    ...
    for word in text.split(" "): #splitting the text on spaces, iterate over each word
        if (word not in abrev) and (word not in punc) and (word not in numbers):
...
if __name__ == "__main__":
    ...
    if not testing:
        ...
        for sent in syllable_condenser(sample_text):
            for syll in enumerate(sent):
                print(syll)
            for syll in sent:
                if long_by_position(syll, sent):
